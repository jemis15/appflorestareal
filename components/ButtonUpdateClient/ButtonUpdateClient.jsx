...
import { Search as IconSearch, Close as IconClose } from "../SVGIcons";
...
const ButtonUpdateClient = ({ client_id, onSuccessUpdate }) => {
  return (
    <Popup
      trigger={
        <button className="bg-gray-200 px-3 py-2 rounded-md hover:bg-gray-300">
          <BaseText size="value">Editar</BaseText>
        </button>
      }
      ...
      {(close) => (
        <ContentUpdateClient
          close={(type) => {
            ...
            const continueClose = confirm("¿Seguro que quieres salir?");
            if (continueClose) {
              close();
            }
          }}
...
const ContentUpdateClient = ({ close, client_id, onSuccessUpdate }) => {
  const { query } = useRouter();
  const { project: project_id } = query;
  const { project } = useProject({ project_id });
  const [loading, setLoading] = useState(false);
  const [formData, setFormData] = useState(null);
  const [tabSelect, setTabSelect] = useState("datos_generales");
  ...
  useEffect(() => {
    const getClient = async () => {
      setLoading(true);
      try {
        const { data: apiData } = await axios.get(`/api/clients/${client_id}`);
        setFormData(apiData.data);
        if (apiData.data.type_document === "ruc") {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      await axios.put(`/api/clients/${client_id}`, formData);
      close("success_update");
      onSuccessUpdate();
    } catch (error) {
      console.log(error);
  ...
  if (loading) {
    return (
      <div className="fixed top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-white">
        <div className="px-5 py-2">Cargando...</div>
      </div>
    );
  }
